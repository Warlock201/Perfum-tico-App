...
print("fun getNoteImageUrl(noteName: String): String {")
print("    val normalized = noteName.trim().lowercase().replace(\"á\", \"a\").replace(\"ç\", \"c\").replace(\"ã\", \"a\").replace(\"ê\", \"e\").replace(\"é\", \"e\").replace(\"í\", \"i\").replace(\"ó\", \"o\").replace(\"ô\", \"o\").replace(\"ú\", \"u\")")
print("    return when {")
for key, title in notes.items():
    img = get_image(title)
    if img:
        print(f"        \"{key}\" in normalized -> \"{img}\"")
print("        else -> \"https://images.unsplash.com/photo-1615599814502-d8a43f87b8f0?w=200&h=200&fit=crop\"")
print("    }")
print("}")
